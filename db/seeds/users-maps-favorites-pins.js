exports.seed = function(knex, Promise) {
  return knex.raw('TRUNCATE favorite_maps, map_pins, maps, users RESTART IDENTITY CASCADE;')
    .then(function () {
      return Promise.all([
        knex('users').insert({first_name: 'Andrew', username: 'theLornNelson'}),
        knex('users').insert({first_name: 'Tess', username: 'tesstess'}),
        knex('users').insert({first_name: 'Lila', username: 'l33loo'})
      ]);
    })
    .then(function () {
      return Promise.all([
    ...
    .then(function() {
      return Promise.all([
        ...
        knex('map_pins').insert({map_id: 1, created_by: 2, latitude: 48.4335921, longitude: -123.3123138, title: "Willows Beach", description: "Awesome swimming!!!"}),
        ...
        knex('map_pins').insert({map_id: 3, created_by: 3, latitude: 48.432082, longitude: -123.3671957, title: "Discovery Coffee", description: "Nice creama, high quality presentation, aromatic."}),
        ...
        knex('map_pins').insert({map_id: 3, created_by: 3, latitude: 48.4286317, longitude: -123.3698819, title: "Habit Coffee", description: "Fantastic coffee order categorization system!"}),
        knex('map_pins').insert({map_id: 4, created_by: 1, latitude: 26.8849344, longitude: 75.5103745, title: "Jaipur, Rajasthan", description: "Capital of Rajasthan; location of the 'Pink City."}),
        knex('map_pins').insert({map_id: 4, created_by: 2, latitude: 24.6082214, longitude: 73.5669747, title: "Udaipur, Rajasthan", description: "Formerly known as the capital of the Mewar Kingdom."}),
